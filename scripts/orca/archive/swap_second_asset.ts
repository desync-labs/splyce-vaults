...
// Swap Vaults, tokenA is devTMAC, tokenB is devUSDC
...
async function main() {
  try {
    ...
    const depositAmount = new BN(20).mul(new BN(10).pow(new BN(6))); // x devUSDC
  ...
  //check the balance of devUSDC in the strategy token account before updating debt
    ...
    const strategyTMACAccount = Keypair.generate();
    console.log("Strategy TMAC token account address:", strategyTMACAccount.publicKey.toBase58());
    ...
    const createAccountIx = SystemProgram.createAccount({
      ...
      newAccountPubkey: strategyTMACAccount.publicKey,
    ...
    await provider.sendAndConfirm(tx, [admin, strategyTMACAccount]);
    
    // Log the newly created account's public key
    console.log("Strategy TMAC token account created successfully:", strategyTMACAccount.publicKey.toBase58())
    ...
    const remainingAccountsForDeployFunds = [
      ...
      {
        pubkey: strategyTMACAccount.publicKey,      // token_owner_account_a (index 2)
        isWritable: true,
        isSigner: false,
      },
      {
    ...
    const strategyTMACAccountBalanceBeforeDeploy = await provider.connection.getTokenAccountBalance(strategyTMACAccount.publicKey);
    console.log("Strategy TMAC account balance before deploying funds:", strategyTMACAccountBalanceBeforeDeploy.value.uiAmount);
    ...
    const strategyTMACAccountBalanceAfterDeploy = await provider.connection.getTokenAccountBalance(strategyTMACAccount.publicKey);
    console.log("Strategy TMAC account balance after deploying funds:", strategyTMACAccountBalanceAfterDeploy.value.uiAmount);
